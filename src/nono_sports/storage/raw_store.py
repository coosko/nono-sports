...
class RawStore:
    def __init__(
        ...
        *,
        ...
        self.raw_root = strava_path(data_root, "raw")
    ...
    def _resolve_relative_path(self, relative_path: str | Path) -> Path:
        ...
        if relative.is_absolute():
            raise ValueError("Raw paths must be relative to the Strava raw root.")
        root = self.raw_root.resolve()
        target = (root / relative).resolve()
        if not target.is_relative_to(root):
            raise ValueError("Raw paths must stay inside the Strava raw root.")
        return target
